...
export class BindThisUtil {
    static bindClassMethod<T extends object>(instance: T): T {
        ...
        Reflect.ownKeys(Reflect.getPrototypeOf(instance))
            .filter((field) => !excludeFieldSet.has(field) && instance[field as keyof T] instanceof Function)
            .forEach((_field) => {
                const field = _field as keyof T
                Reflect.set(instance, field, ((instance[field] as any) as Func).bind(instance))
            })
        return instance
    }
}
