...
function watchPostEffect(effect2, options) {
  return doWatch(
    effect2,
    null,
    true ? Object.assign(options || {}, { flush: 'post' }) : { flush: 'post' },
  )
}
function watchSyncEffect(effect2, options) {
  return doWatch(
    effect2,
    null,
    true ? Object.assign(options || {}, { flush: 'sync' }) : { flush: 'sync' },
  )
}
  ...
  'node_modules/@vue/runtime-core/dist/runtime-core.esm-bundler.js'() {
    ...
    deprecationData = {
      ...
      ['INSTANCE_EVENT_HOOKS']: {
        message: (event) =>
          `"${event}" lifecycle events are no longer supported. From templates, use the "vnode" prefix instead of "hook:". For example, @${event} should be changed to @vnode-${event.slice(
            5,
          )}. From JavaScript, use Composition API to dynamically register lifecycle hooks.`,
...
`
  ...
  (This can also be done per-component via the "compatConfig" option.)`,
      ...
      ['PRIVATE_APIS']: {
        message: (name) =>
          `"${name}" is a Vue 2 private API that no longer exists in Vue 3. If you are seeing this warning only due to a dependency, you can suppress this warning via { PRIVATE_APIS: 'supress-warning' }.`,
      },
    }
...
//# sourceMappingURL=chunk-SEDOLZXP.js.map
